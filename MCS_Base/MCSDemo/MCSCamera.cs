...
namespace MCS
{
    ...
    public class MCSCamera
    {
        ...
        public void AddNewRulerCamera(string name, string ip)
        {
            RulerCamera Ruler = new RulerCamera();
            Rulers.Add(Ruler);
            Ruler.Startup(name, Rulers.Count, ip, null, null, null);
            System.Threading.Thread.Sleep(1000);
            SetConnectionState(CameraConnectionState, true);
            SetCaptureState(CameraCaptureState, true);
        }

        public void RemoveLastRulerCamera()
        {
            if (Rulers.Count > 0)
            {
                RulerCamera lastRuler = Rulers.Last();
                Rulers.Remove(lastRuler);
                lastRuler.Shutdown();

                StatusStorage.RemoveCamera("Camera." + lastRuler.CameraName + ".ConnState");
                StatusStorage.RemoveCamera("Camera." + lastRuler.CameraName + ".CaptureState");
            }
        }
